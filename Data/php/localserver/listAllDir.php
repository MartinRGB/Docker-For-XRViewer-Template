...
// # function 0
    $folder          = "/wwwroot/127.0.0.1/next/$routerPath";
    $return_array = array();
            ...
            foreach (new DirectoryIterator($dir) as $f) {
                    ...
                    if ($f->isFile()) {
                        if (str_contains($name, '.mdx')) {
                            ...
                            if($rPath === "docs"){
                                $finalRoute2 = str_replace($folder,"",$finalRoute);
                                $finalRoute3 = str_replace("\\","/",$finalRoute2);
                                $finalRoute4 = str_replace(".mdx","",$finalRoute3);
                                $data[] = [ 
                                    'kind' => 'MdxPage',
                                    // 'file' => $name
                                    'name' => str_replace(".mdx","",$name),
                                    // todo get request file path;
                                    'route' => "/$rPath" . substr($finalRoute4, 0, -1),
        
                                    'time' => "$fileT",
        
                                    "information" => "/$rPath" . $finalRoute4 . "frontmatter.md",
                                    //'route' => $path
                                ];
                            }
                            
                            if($rPath === "tools"){
                                $finalRoute2 = str_replace($folder,"",$filted_path);
                                $finalRoute3 = str_replace("\index.html","/",$finalRoute2);
                                $finalRoute4 = str_replace("\\","/",$finalRoute3);
                                $finalRoute5 = str_replace("////","/",$finalRoute4);
                                $finalRoute6 = str_replace("index.mdx","",$finalRoute5);
                                $data[] = [ 
                                    'kind' => 'MdxPage',
                                    // 'file' => $name
                                    'name' => str_replace(".mdx","",$name),
                                    // todo get request file path;
                                    'route' => "/$rPath" . $finalRoute6 . str_replace(".mdx","",$name),
        
                                    'time' => "$fileT",
        
                                    "information" => "/$rPath" . $finalRoute6 . "frontmatter.md",
                                    //'route' => $path
                                ];
                            }
                        ...
                        if (str_contains($name, 'index.html')) {
                            ...
                            $data[] = [ 
                                ...
                                'name' => 'index.html',
                                // todo get request file path;
                                'route' => "/$rPath" . $finalRoute5,

                                'time' => "$fileT",

                                "cover" => "/$rPath" . $finalRoute5 . "cover.png",

                                "information" => "/$rPath" . $finalRoute5 . "frontmatter.md",
                                //'route' => $path
                            ];
                        }
